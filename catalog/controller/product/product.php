...
class ControllerProductProduct extends Controller {
	...
	public function index() { 
        ...
        $this->document->addScript('catalog/view/javascript/jquery/jquery.tinycarousel.min.js');
		...
		if ($product_info) {
            ...
            if ((float)$product_info['special']) {
					if ($product_info['price'] > 0) {
							$this->data['sale'] = '<div class="stiker-special-product"></div>';
					} else {
            ...
            $set1 = $this->config->get('config_image_popup_width') + 200; // Ўирина зум изображени€ дополнительных фото
            $set2 = $this->config->get('config_image_popup_height')/$this->config->get('config_image_popup_width')*$set1; // ¬ысота зум изображени€ дополнительных фото
			            
            if ($product_info['image']) {
				$this->data['popup1'] = $this->model_tool_image->resize($product_info['image'], $set1, $set2);
			} else {
				$this->data['popup1'] = '';
			}
            ...
            $set3 = $this->config->get('config_image_thumb_width') + 200; // Ўирина зум изображени€ дополнительных фото
            $set4 = $this->config->get('config_image_thumb_height')/$this->config->get('config_image_thumb_width')*$set3; // ¬ысота зум изображени€ дополнительных фото
			...
			foreach ($results as $result) {
				$this->data['images'][] = array(
                    ...
                    'thumb2' => $this->model_tool_image->resize($result['image'], $set3, $set4)
				);
			}	
